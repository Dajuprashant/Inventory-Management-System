...
def whenPurchaseItem():
    ...
    title = Label(purchase, text="Purchase Item", font=(
        "Arial", 23), bg="#545454", fg="white").pack(pady=(15, 0))

    # for text field labels
    product_name_label = Label(purchase, text="Product Name",
                               bg="#545454", fg="white", font=("Arial", 10)).place(x=70, y=95)
    quantity_label = Label(purchase, text="Quantity", bg="#545454",
                           fg="white", font=("Arial", 10)).place(x=320, y=95)
    costprice_label = Label(purchase, text="Cost Price", bg="#545454",
    ...
    # for confirm button
    confirm_button = Button(purchase, text="Confirm Purchase", height=2,
                            width=22, border=0, bg="#004789", fg="white", font=("Arial", 10, "bold"))
    confirm_button.place(x=210, y=230)

    # for total label
    total_purchase = Label(purchase, font=(
        "Arial", 16), bg="#545454", fg="white")
    total_purchase.place(x=450, y=234)

    def calculate_total(*args):
        try:
            totalPurchaseAmt = int(costprice_var.get()) * \
...
def whenSaleItem():
    ...
    title = Label(sale, text="Sale Item", font=("Arial", 23),
                  bg="#545454", fg="white").place(x=50, y=20)
    ...
    product_name_label = Label(sale, text="Product Name", bg="#545454", fg="white", font=(
        "Arial", 10)).place(x=170, y=95)
    ...
    # for confirm button
    confirm_button = Button(sale, text="Confirm Sale", height=2, width=22,
                            border=0, bg="#004789", fg="white", font=("Arial", 10, "bold"))
    confirm_button.place(x=210, y=230)
...
search_btn = Button(appBar, image=searchImg, cursor='hand2',
                    relief='sunken', bg="#FF5252", )
search_btn.image = searchImg
search_btn.place(x=740, y=15)

# to add buttons to left
home_btn = Button(container, text="Home", image=homeImg, cursor='hand2', compound="left", height=35, width=260, bg="#FF5252",
                  fg="white", border=0, font=customButtonFont, )
home_btn.image = homeImg
home_btn.pack(pady=(280, 0))

purchase_btn = Button(container, text="Purchase", cursor='hand2', image=dollarImg, compound="left", height=35, width=260, bg="#FF5252",
                      fg="white", border=0, font=customButtonFont, command=whenPurchaseItem)
purchase_btn.image = dollarImg
purchase_btn.pack(pady=(10, 0))

saleBtn = Button(container, text="Sale", image=shoppingImg, cursor='hand2', compound="left", height=35, width=260, bg="#FF5252",
                 fg="white", border=0, font=customButtonFont, command=whenSaleItem)
saleBtn.image = shoppingImg
saleBtn.pack(pady=(10, 0))

account_btn = Button(container, text="Account", image=userImg, cursor='hand2', compound="left", height=35, width=260, bg="#FF5252",
                     fg="white", border=0, font=customButtonFont, )
account_btn.image = userImg
account_btn.pack(pady=(10, 0))

# for avialbel stocks label
Label(root, text="Availabe Stocks", font=(
    "Arial", 18, "bold"), bg="#8A908B").place(x=320, y=95)
...
id_label = Label(stocks_frame_labels, text="ID", font=(
    "Arial", 13, "bold",), bg="#D9D9D9").grid(row=0, column=0, padx=(0, 52))
productName_label = Label(stocks_frame_labels, text="Product Name", font=(
    "Arial", 13, "bold"), bg="#D9D9D9").grid(row=0, column=1, padx=(0, 168))
qty_label = Label(stocks_frame_labels, text="Qty", font=(
    "Arial", 13, "bold"), bg="#D9D9D9").grid(row=0, column=2, padx=(0, 70))
cp_label = Label(stocks_frame_labels, text="CP", font=(
    "Arial", 13, "bold"), bg="#D9D9D9").grid(row=0, column=3, padx=(0, 76))
total_label = Label(stocks_frame_labels, text="Total", font=(
    "Arial", 13, "bold"), bg="#D9D9D9").grid(row=0, column=4, padx=(0, 0))
